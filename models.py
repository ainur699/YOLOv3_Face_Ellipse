...
def YoloFaceOutput(filters, anchors, name=None):
    def yolo_output(x_in):
        x = inputs = Input(x_in.shape[1:])
        x = DarknetConv(x, filters * 2, 3)
        x = DarknetConv(x, anchors * 6, 1, batch_norm=False)
        x = Lambda(lambda x: tf.reshape(x, (-1, tf.shape(x)[1], tf.shape(x)[2], anchors, 6)))(x)
        return tf.keras.Model(inputs, x, name=name)(x_in)
    return yolo_output


def yolo_boxes(pred, anchors):
    # pred: (batch_size, grid, grid, anchors, (obj, x, y, w, h))
    grid_size = tf.shape(pred)[1]
    objectness, box_xy, box_wh, angle = tf.split(pred, (1, 2, 2, 1), axis=-1)

    box_xy = tf.sigmoid(box_xy)
    objectness = tf.sigmoid(objectness)
    angle = tf.atan(angle)
    pred_box = tf.concat((box_xy, box_wh, angle), axis=-1)  # original xywh for loss
...
def yolo_nms(outputs, anchors, masks):
    ...
    score = tf.reshape(confidence, (tf.shape(confidence)[0], -1))
    ...
    for i in tf.range(tf.shape(bbox)[0]):
        ...
        selected_boxes = tf.gather(bbox_xywh[i], selected_indices)
        selected_angles = tf.gather(angles[i], selected_indices)
        selected_scores = tf.gather(confidence[i], selected_indices)

        out = tf.concat([selected_boxes, selected_angles, selected_scores], axis=-1)
        paddings = [[0, FLAGS.yolo_max_boxes - tf.shape(out)[0]], [0, 0]]
...
def YoloV3Face(size=None, channels=3, anchors=yolo_face_anchors, masks=yolo_face_anchor_masks, training=False):
    ...
    x = YoloConv(512, name='yolo_conv_0')(x)
    output_0 = YoloFaceOutput(512, len(masks[0]), name='yolo_output_0')(x)

    x = YoloConv(256, name='yolo_conv_1')((x, x_61))
    output_1 = YoloFaceOutput(256, len(masks[1]), name='yolo_output_1')(x)

    x = YoloConv(128, name='yolo_conv_2')((x, x_36))
    output_2 = YoloFaceOutput(128, len(masks[2]), name='yolo_output_2')(x)

    if training:
        return Model(inputs, (output_0, output_1, output_2), name='yolov3')

    boxes_0 = Lambda(lambda x: yolo_boxes(x, anchors[masks[0]]), name='yolo_boxes_0')(output_0)
    boxes_1 = Lambda(lambda x: yolo_boxes(x, anchors[masks[1]]), name='yolo_boxes_1')(output_1)
    boxes_2 = Lambda(lambda x: yolo_boxes(x, anchors[masks[2]]), name='yolo_boxes_2')(output_2)

    outputs = Lambda(lambda x: yolo_nms(x, anchors, masks), name='yolo_nms')((boxes_0[:3], boxes_1[:3], boxes_2[:3]))
...
def YoloV3FaceTiny(size=None, channels=3, anchors=yolo_face_tiny_anchors, masks=yolo_face_tiny_anchor_masks, training=False):
    x = inputs = Input([size, size, channels], name='input')

    x_8, x = DarknetTiny(name='yolo_darknet')(x)

    x = YoloConvTiny(256, name='yolo_conv_0')(x)
    output_0 = YoloFaceOutput(256, len(masks[0]), name='yolo_output_0')(x)

    x = YoloConvTiny(128, name='yolo_conv_1')((x, x_8))
    output_1 = YoloFaceOutput(128, len(masks[1]), name='yolo_output_1')(x)
    ...
    outputs = Lambda(lambda x: yolo_nms(x, anchors, masks), name='yolo_nms')((boxes_0[:3], boxes_1[:3]))
...
def YoloLoss(anchors, ignore_thresh=0.5):
    def yolo_loss(y_true, y_pred):
        ...
        true_box = tf.concat([box_x1y1, box_x2y2], axis=-1)
        ...
        angle_loss = obj_mask * tf.reduce_sum(tf.square(true_angle - pred_angle), axis=-1)
        ...
        angle_loss = tf.reduce_sum(angle_loss, axis=(1, 2, 3))
        obj_loss = tf.reduce_sum(obj_loss, axis=(1, 2, 3))

        return xy_loss + wh_loss + angle_loss + obj_loss
    return yolo_loss
